...
export function AuthContainer() {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('✅ 5. Compte créé, attente 500ms...');
      await new Promise((resolve) => setTimeout(resolve, 500));

      console.log('🔵 6. Appel signIn...');
      const result = await signIn('credentials', {
      ...
      console.log('📊 7. Résultat signIn:', result);
      ...
      if (result?.error) {
        console.log('❌ 8. Erreur signIn:', result.error);
        setIsSignUp(false);
      ...
      if (result?.ok) {
        console.log('✅ 9. SignIn OK !');
        console.log('🚀 10. REDIRECTION vers /home...');

        // Essayer plusieurs méthodes de redirection
        try {
          window.location.href = '/home';
        } catch (redirectError) {
          console.error('❌ Erreur window.location:', redirectError);
          // Fallback
          router.push('/home');
          router.refresh();
        }
      ...
      console.log('⚠️ 11. Résultat inattendu, pas d\'erreur mais pas OK non plus');
      console.log('Tentative de redirection quand même...');
      window.location.href = '/home';
